# -*- coding: utf-8 -*-
"""
Created on Mon Apr  9 11:52:57 2018
...
handled['hour'] = handled['date_time'].apply(lambda x: datetime.strptime(x, '%Y-%m-%d %H:%M:%S').hour)

#%%

hour_mask = handled['hour'] < 12

handled = handled[hour_mask, :]
...
ticket_dataset = pd.read_csv("C:\\Users\\emota\\Documents\\DataScience\\phone_tickets_apr_9.csv")
...
ticket_dataset['hour'] = ticket_dataset['create_time'].apply(lambda x: datetime.strptime(x, '%Y-%m-%d %H:%M:%S').hour)
...
hour_mask = ticket_dataset['hour'] < 15

#%%

ticket_day = ticket_dataset.loc[hour_mask,:]

#%%

by_program = ticket_day.groupby(['date', 'value_text'])['id'].count().unstack()
...
for p in programs:
	...
	if by_program[p].sum() > 100:
		...
		if zscore >= 1.65:
			...
			program_mask = ticket_day['value_text'] == p

			program_set = ticket_day.loc[program_mask,:]

			by_reason = program_set.groupby(['date', 'service_id'])['id'].count().unstack()
			...
			for r in reasons:
				...
				if by_reason[r].sum() > 100:
					zscore_d = 0
					zscore_d = ((by_reason.iloc[-1,:][r]-d_data[r].mean())/d_data[r].std())
					print (reasons_dataset.loc[r], zscore)
					if p == 'Immunotec':
						print (reasons_dataset.loc[r], zscore_d)
						p_d = by_reason[r]
						p_d.plot()
						plt.show()
						i = input("continue")
